...
void MCurveMeshOverride::clear_override_no_signal(int64_t id){
    data.erase(id);
}
...
PackedByteArray MCurveMeshOverride::get_data() const {
    PackedByteArray out;
    int sb = sizeof(Pair<int64_t,Override>);
    int counter = 0;
    for(HashMap<int64_t,Override>::ConstIterator it=data.begin();it!=data.end();++it){
        int pos = counter*sb;
        out.resize(pos+sb);
