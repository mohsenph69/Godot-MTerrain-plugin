    ...
    for(int c=0; c < conn_ids.size(); c++){
        ...
        for(int i=0; i < interval_count; i++){
            ...
            while (side_dis <= total_dis)
            {
                ...
                float t = UtilityFunctions::smoothstep(total_dis,_paint_radius,std::abs(side_dis));
                h = (ppp.y - h)*t + h;
                grid->set_height_by_pixel(t_px.x,t_px.y,h);
                side_dis += interval_meter;
            }
    ...
    for(int c=0; c < conn_ids.size(); c++){
        ...
        for(int i=0; i < interval_count; i++){
            ...
            while (side_dis <= total_dis)
            {
                ...
                processed_px.insert(t_px);
                float t = UtilityFunctions::smoothstep(total_dis,_paint_falloff,std::abs(side_dis));
