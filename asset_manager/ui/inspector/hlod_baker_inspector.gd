...
func _ready():	
	...
	%edit_scene_in_blender_button.toggled.connect(func(toggle_on): 
		%tcp_panel.visible = toggle_on
		TCPIO.show_controls = toggle_on
		if toggle_on:
			open_baker_scene_with_blender()
	)
	%edit_scene_in_blender_button.button_pressed =TCPIO.show_controls
	%tcp_panel.visible = TCPIO.show_controls
	
	if TCPIO.server:		
		%server_toggle_button.button_pressed = true
		%server_toggle_button.text = "ON"
		if TCPIO.timer:
			%manual_tcp_hbox.visible = true
	else:
		%manual_tcp_hbox.visible = false
	%server_toggle_button.toggled.connect(func(toggle_on):		 
		%server_toggle_button.text = "ON" if toggle_on else "OFF"
		if toggle_on:			
			TCPIO.start_server()		
			if TCPIO.auto:
				TCPIO.start_timer()			
	...
	%automatic_tcp_toggle_button.toggled.connect(func(toggle_on):
		TCPIO.auto = toggle_on
		%automatic_tcp_toggle_button.text = "A" if toggle_on else "M"
		%manual_tcp_hbox.visible = not toggle_on and TCPIO.server
		if toggle_on:
			TCPIO.start_timer()		
	)	
	%automatic_tcp_toggle_button.button_pressed = TCPIO.auto	
	%tcp_sync_button.pressed.connect(TCPIO.update_client_list)
	%tcp_send_button.pressed.connect(TCPIO.send_update)
	%tcp_receive_button.pressed.connect(TCPIO.receive_messages)
