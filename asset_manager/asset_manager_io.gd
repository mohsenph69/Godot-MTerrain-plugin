...
static func get_asset_library():	
	...
	config.load("res://addons/m_terrain/config/asset_manager.cfg")
	var path = config.get_value("paths", "asset_library", "res://addons/m_terrain/asset_manager/asset_library.res")
	var asset_library
...
static func update_objects_from_glb(asset_library:MAssetTable, path):
	...
	for object in scene:
		...
		if "static_body" in extras:
			var collection_id = asset_library.collection_create(object.name)						
			var mesh_children = []						
			...
			var mesh_item_ids = import_mesh_item_from_nodes(asset_library, mesh_children)			
	...
	for child:Node in nodes:			
		...
		if "_lod_" in child.name:				
			...
			if child in all_mesh_lod_nodes:					
				...
				mesh_item_names.push_back(child.name.split("_lod_")[0])
	#Create single item collections	
	for i in mesh_item_ids.size():
		#if asset_library.has_collection()
		var collection_id = asset_library.collection_create(mesh_item_names[i])
		asset_library.collection_add_item(collection_id, MAssetTable.MESH, mesh_item_ids[i], Transform3D())
	return mesh_item_ids
